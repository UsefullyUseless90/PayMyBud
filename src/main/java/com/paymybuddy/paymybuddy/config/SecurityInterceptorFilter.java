...
public class SecurityInterceptorFilter implements HandlerInterceptor {
    ...
    public void restoreApplicationContext(UtilisateurDTO user, HttpServletRequest request) {

        List<GrantedAuthority> authorities = customUserDetailsService.getUserAuthority(user.roles);
        UserDetails userDetails =  new CustomUserDetails(
                new org.springframework.security.core.userdetails.User(
                        user.getNomPrenom(), user.getMotDePasse(),
    ...
    public void saveUserInSession(UtilisateurDTO user, HttpServletRequest request){
        HttpSession session = request.getSession();
